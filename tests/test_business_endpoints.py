...
def test_semantic_search_endpoint_returns_expected_structure(monkeypatch):
    def fake_semantic_search(request):
    ...
    monkeypatch.setattr(business_router, "semantic_search", fake_semantic_search)
...
def test_semantic_search_endpoint_respects_response_schema(monkeypatch):
    def fake_semantic_search(request):
        return SemanticSearchResponse(
            query=request.query,
            results=[
    ...
    assert set(data["results"][0].keys()) == {"source", "score", "snippet"}
